...
WORKDIR=$HOME
SRC_FOLDER=$WORKDIR/awg
DEST_FOLDER=$WORKDIR/awg_$ARCH
...
$MAKE_BIN 
$MAKE_BIN  install
# goto BUILD_FOLDER and go on build
cd $BUILD_FOLDER
...
mkdir $DEST_FOLDER
cp $BUILD_FOLDER/awg.exe $DEST_FOLDER/
...
$STAGING/$MINGW_VERSION/bin/windeployqt.exe --dir $DEST_FOLDER --no-system-d3d-compiler --no-compiler-runtime --no-quick-import --opengl --printsupport $BUILD_FOLDER/awg.exe
cp -r $STAGING/$MINGW_VERSION/share/libsigrokdecode/decoders  $DEST_FOLDER/

#tar -C /c/$DEST_FOLDER --strip-components=3 -xJf /c/awg-$MINGW_VERSION-build-deps.tar.xz msys64/$MINGW_VERSION/bin
...
#/$MINGW_VERSION/bin/objcopy -v --only-keep-debug /c/$DEST_FOLDER/awg.exe /c/$DEST_FOLDER/.debug/awg.exe.debug
$STAGING/$MINGW_VERSION/bin/dump_syms -r $DEST_FOLDER/awg.exe > $DEST_FOLDER/awg.exe.sym
#/c/msys64/$MINGW_VERSION/bin/strip.exe --strip-debug --strip-unneeded /c/$DEST_FOLDER/awg.exe
#/c/msys64/$MINGW_VERSION/bin/strip.exe --strip-debug --strip-unneeded /c/$DEST_FOLDER/*.dll
#/c/msys64/$MINGW_VERSION/bin/objcopy.exe -v --add-gnu-debuglink=/c/$DEST_FOLDER/.debug/awg.exe.debug /c/$DEST_FOLDER/awg.exe
mkdir $DEBUG_FOLDER
mv $DEST_FOLDER/awg.exe.sym $DEBUG_FOLDER
mv $DEST_FOLDER/.debug $DEBUG_FOLDER
$TOOLS_FOLDER/cv2pdb/cv2pdb $DEST_FOLDER/awg.exe
...
cp -R $DEST_FOLDER/awg $DEBUG_FOLDER/awg
...
cd $WORKDIR
zip -r "$ARTIFACT_FOLDER/awg-${ARCH}.zip" awg_$ARCH
zip -r "$ARTIFACT_FOLDER/debug-${ARCH}.zip" debug_$ARCH
PATH=/c/innosetup:$PATH
iscc //p $BUILD_FOLDER/awg-$ARCH_BIT.iss
mv $WORKDIR/awg-$ARCH_BIT-setup.exe $ARTIFACT_FOLDER
